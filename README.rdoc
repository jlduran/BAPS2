...
2. Connect a RS232 cable to your IP04 (via the daughter board) and stop the boot process at the u-boot prompt.
   ...
   Install the new uImage into NAND flash using u-boot:
     ...
     ip04>set bootargs ethaddr=$(ethaddr) ethaddr1=$(ethaddr1) console=ttyBF0,115200 root=/dev/mtdblock0 rw
     ip04>save
...
4. Now set up u-boot to mount root from yaffs (some of these env variables may be set already, use _print_ to check):

     ip04>set autostart yes
     ip04>set bootargs ethaddr=$(ethaddr) ethaddr1=$(ethaddr1) console=ttyBF0,115200 root=/dev/mtdblock2 rw
...
documentation files can also be browsed from BAPS2 git[http://svn.astfin.org/software/baps/trunk/doc].
...
2. You need this toolchain:
     ...
     [BAPS2]$ wget http://blackfin.uclinux.org/gf/download/frsrelease/ \
             348/3347/blackfin-toolchain-07r1.1-3.i386.tar.gz
     [BAPS2]$ tar xzf blackfin-toolchain-07r1.1-3.i386.tar.gz
...
uClinux before making any other packages.
  ...
  Or you can flash the uImage as described in the Getting Started[local://getting_started] section above.
...
Now rcp libg729.so from libbfgdots to /lib on your IP04. When Asterisk boots it should load the g729 codec.  Check the codec is installed with 'core show translate' at the Asterisk CLI.
