...
TARGET_DIR=$(TOPDIR)/tmp/voiptel-gui/ipkg/voiptel-gui
VOIPTELGUI_DIR=$(BUILD_DIR)/voiptel-gui

PKG_NAME:=voiptel-gui
PKG_VERSION:=1.2.5
PKG_RELEASE:=1
PKG_BUILD_DIR:=$(TOPDIR)/tmp/voiptel-gui
...
voiptel-gui:
	...
	# Note we install to /etc/asterisk.voiptel to avoid clash with existing /etc/asterisk
...
define Package/voiptel-gui
  ...
  MAINTAINER:=Voiptel
  DESCRIPTION:=\
        VoIPtel GUI.  An upgraded and debugged	\
...
define Package/voiptel-gui/postinst
#!/bin/sh

# replace conf files without ipkg installer complaining

rm -r /etc/asterisk.backup
...
define Package/voiptel-gui/prerm
#!/bin/sh

echo "NOTE: Previous version of the Asterisk configuration files may be found in /etc/asterisk.backup."
...
$(eval $(call BuildPackage,voiptel-gui))

voiptel-gui-package: voiptel-gui $(PACKAGE_DIR)/voiptel-gui_$(VERSION)_$(PKGARCH).ipk
