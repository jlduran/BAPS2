...
static inline void wcfxs_transmitprep(struct wcfxs *wc, u8 *writechunk)
{
	...
	for (x=0;x<ZT_CHUNKSIZE;x++) {
		...
		if (wc->cardflag & (1 << 0)) {
			writechunk[8*x+card2TDMslot_mapping[0]] = wc->chans[0].writechunk[x];//YN temp[x];//
...
static int wcfxs_powerup_proslic(struct wcfxs *wc, int card, int fast)
{
	...
	printk("reg 0: 0x%x \n", wcfxs_getreg(wc, card, 0));//to be left
...
//YN
//TDM_channel_offset parameter added
static int wcfxs_init_proslic(struct wcfxs *wc, int card, int TDM_channel_offset,int fast, int manual, int sane)
{
...
static int wcfxs_hooksig(struct zt_chan *chan, zt_txsig_t txsig)
{
	...
	if (wc->modtype[chan->chanpos - 1] == MOD_TYPE_FXS) {//YN
		...
		case ZT_TXSIG_ONHOOK:
			...
			case ZT_SIG_EM:
				if (debug)
			    		printk("ZT_SIG_EM FXS %d\n", chan->chanpos - 1);
			case ZT_SIG_FXOKS:
				if (debug)
			    		printk("ZT_SIG_FXOKS FXS %d\n", chan->chanpos - 1);
			case ZT_SIG_FXOLS:
				if (debug)
			    		printk("ZT_SIG_FXOLS FXS %d\n", chan->chanpos - 1);
				wc->mod.fxs.lasttxhook[chan->chanpos-1] = wc->mod.fxs.idletxhookstate[chan->chanpos-1];
				break;
			case ZT_SIG_FXOGS:
				if (debug)
			    		printk("ZT_SIG_FXOGS %d\n", chan->chanpos - 1);
	...
	if ((wc->modtype[chan->chanpos - 1] == MOD_TYPE_FXO) && (port_type[chan->chanpos - 1] == 'G')) {//YN
		...
		case ZT_TXSIG_START:
			...
			if(strlen(chan->txdialbuf)!=0){
				...
				chan->rxhooksig = ZT_RXSIG_OFFHOOK;// YN
		...
		case ZT_TXSIG_OFFHOOK:
			if(incomming_call_state==1){//During ringing zaptel gives OFFHOOK
					incomming_call_state=2;
					GSM_send(wc,chan->chanpos - 1,"ata");//was GSM_send(wc,0,"ata");//Answer the call
			}
			if (debug)
			    printk("ZT_TXSIG_OFFHOOK GSM %d\n", chan->chanpos - 1);
			wc->mod.fxo.offhook[chan->chanpos - 1] = 1;
			break;
		case ZT_TXSIG_ONHOOK:
...
static int wcfxs_init_one(struct wcfxs_desc *d)
{
	...
	//Generate the SIM PIN enter command string
	strcat(SIM_Enter_string,SIM_pin);
	strcat(SIM_Enter_string,"\"");
	printk("SIM PIN entered => %s\n",SIM_pin);
	//printk("SIM string => %s\n",SIM_Enter_string);
