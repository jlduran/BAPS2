...
//	Tooltips for users.html in english
	...
	tooltips['users'] .en[0] = "<B>Extension:</B> The numbered extension, i.e. 234, that will be associated with this particular User / Phone." ;  //Extension
	...
	tooltips['users'] .en[6] = "<B>Dial Plan:</B> Please choose the Dial Plan for this user as defined under the \"Calling Rules\" option to the left."; //Calling Rules
	...
	tooltips['users'] .en[20] = "<B>Can Reinvite:</B> Set up calls directly between caller and receiver, after being connected by Asterisk. This method is known to cause problems with certain hardware, such as the common Cisco ATA 186.";
	tooltips['users'] .en[21] = "<B>NAT:</B> Check this option if the User or Phone is located behind a NAT (Network Address Translation) enabled gateway.";
	tooltips['users'] .en[22] = "<B>DTMFMode:</B> The Dual-Tone Multi-Frequency mode to be used is specified here and can be changed if necessary. The default is rfc2833.";
	tooltips['users'] .en[23] = "<B>Codecs:</B> The allowed and disallowed codecs can be selected by clicking this link. By default only alaw and ulaw are allowed.";
...
// Tooltips for SIP_General (sip_general)
	...
	tooltips['callingrules'].en[10] = "<B>Analog fallback:</B> Select which analog line to fall back on in case the service provider above cannot be reached";
	...
	tooltips['network']= new Object;
	tooltips['network'].en = new Array;
	tooltips['network'].en[0] = "<B>DHCP:</B> Receive IP address dynamically from a DHCP server";
	tooltips['network'].en[1] = "<B>Hostname:</B> The name identifying this machine on the network";
	tooltips['network'].en[2] = "<B>Domain:</B> The name of the network domain that this machine belongs to";
	tooltips['network'].en[3] = "<B>IP address:</B> Use this field to set a static IP address on this machine";
	tooltips['network'].en[4] = "<B>Subnet mask:</B> Specify the subnet mask of the static IP address above";
	tooltips['network'].en[5] = "<B>Gateway:</B> Specify a gateway to reach non-local addresses";
	tooltips['network'].en[6] = "<B>DNS:</B> Specify a nameserver to resolve domain names";
	tooltips['network'].en[7] = "<B>Country:</B> Select your country so that the system may use the proper tone indications";
	tooltips['network'].en[8] = "<B>Save:</B> Save the settings";
	tooltips['network'].en[9] = "<B>Cancel:</B> Reset to previously saved settings";
	tooltips['network'].en[10] = "<B>NTP:</B> Specify the NTP server that you wish to use. You may type either the domain name or the IP address of the server, and it may be either remote or local. The default server is pool.ntp.org. Be aware that the PBX needs to be able to access a NTP server in order to function properly.";

	tooltips['update']= new Object;
	tooltips['update'].en = new Array;
	tooltips['update'].en[0] = "<B>Update:</B> Click this button to download and install any available software updates.";